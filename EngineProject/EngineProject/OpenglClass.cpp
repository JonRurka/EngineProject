...
void OpenGLClass::InitializeLog(Logging* logging)
{
	// Stor a pointer to the logging objects.
	m_Logging = logging;
}
...
bool OpenGLClass::InitializeOpenGL(HWND hwnd, int screenWidth, int screenHeight, float screenDepth, float screenNear, bool vsync)
{
	...
	if (!m_deviceContext)
	{
		MessageBox(hwnd, L"InitializeOpenGL: Failed to get Device Context.", L"Error", MB_OK);
		return false;
	}
	...
	if (result != 1)
	{
		MessageBox(hwnd, L"InitializeOpenGL: Failed to get pixel format.", L"Error", MB_OK);
		return false;
	}
	...
	if (result != 1)
	{
		MessageBox(hwnd, L"InitializeOpenGL: Failed to set pixel format.", L"Error", MB_OK);
		return false;
	}
	...
	{
		MessageBox(hwnd, L"InitializeOpenGL: Failed to create rendering context.", L"Error", MB_OK);
		return false;
	}
	...
	if (result != 1)
	{
		MessageBox(hwnd, L"InitializeOpenGL: Setting rendering context to active failed.", L"Error", MB_OK);
		return false;
	}
	...
	// Set the feild of view and screen aspect ratio.
	...
	m_Logging->Log(m_videoCardDescription);
	...
	if (result != 1)
	{
		MessageBox(hwnd, L"InitializeOpenGL: Vsync not set correctly.", L"Error", MB_OK);
		return false;
	}
...
{
	// Make sure colors are within 0 and 1.
	ClampColor(red, green, blue, alpha);
...
{
	// Presents the back buffer to the screen since rendering is complete.
...
void OpenGLClass::ClampColor(float& R, float& G, float& B, float& A)
{
	if (R>1)
		R=1;
	if (R<0)
